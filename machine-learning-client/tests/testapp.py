...
import pytest
import torch
from camera import (capture_image_from_camera, preprocess_image, load_model,
                    predict, save_prediction, serialize_image)
...
def test_capture_image_from_camera(mock_image):
    ...
    with patch('cv2.VideoCapture') as mock_cap:
        ...
        with patch('cv2.waitKey', side_effect=[ord(' '), 27]):
            captured_image = capture_image_from_camera()
            assert captured_image is not None
            assert captured_image.shape == mock_image.shape
...
def test_save_prediction(mock_image):
